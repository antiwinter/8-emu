...
export const Bracket: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-6xl mx-auto p-4">
      <h2 className="text-3xl font-bold text-yellow-500 mb-8">Knockout Phase</h2>
      ...
      <div className="mt-12">
        <button
          ...
          className={`
            px-12 py-6 bg-gradient-to-r from-yellow-600 to-yellow-800 text-white font-bold text-2xl rounded-full shadow-lg 
          ...
          `}
        >
          {isSimulating ? 'Simulating...' : (!!final?.winnerId ? 'Tournament Finished' : 'Play Next Games')}
        </button>
      </div>
    </div>
  );
...
const SeriesCard: React.FC<SeriesCardProps> = ({ series, getPlayer, title, targetWins, isFinal, userPlayerId }) => {
  ...
  return (
    <div className={`
      flex flex-col bg-gray-800 rounded-xl overflow-hidden border-2 w-full
